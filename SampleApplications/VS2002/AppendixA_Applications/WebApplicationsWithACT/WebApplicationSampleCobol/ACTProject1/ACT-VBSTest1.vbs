...
Sub SendRequest1()
    ...
    Else
        ...
        oRequest.Path = "/WebApplicationSampleCobol/WebForm1.aspx"
        oRequest.Verb = "GET"
        ...
        If (oResponse is Nothing) Then
            Test.Trace "Error: Failed to receive response for URL to " + "/WebApplicationSampleCobol/WebForm1.aspx"
        Else
...
Sub SendRequest2()
    Dim oConnection, oRequest, oResponse, oHeaders, strStatusCode
    If fEnableDelays = True then Test.Sleep (20820)
    Set oConnection = Test.CreateConnection("localhost", 80, false)
    If (oConnection is Nothing) Then
        Test.Trace "Error: Unable to create connection to localhost"
    Else
        ...
        oHeaders.RemoveAll
        oHeaders.Add "Accept", "image/gif, image/x-xbitmap, image/jpeg, image/pjpeg, application/vnd.ms-powerpoint, application/vnd.ms-excel, application/msword, */*"
        oHeaders.Add "Referer", "http://localhost/WebApplicationSampleCobol/WebForm1.aspx"
        oHeaders.Add "Accept-Language", "en-us"
        ...
        'oHeaders.Add "Cookie", "ASP.NET_SessionId=cdwhqnbxs01xonmpln5bsu55"
        oHeaders.Add "Cookie", "(automatic)"
        oHeaders.Add "Content-Length", "(automatic)" 
        oRequest.Body = "__VIEWSTATE=dDw0NDgzMjkxOTk7Oz5Au%2F2Tts6Vhvl1vzUw"
        oRequest.Body = oRequest.Body + "%2B0pj17Zd9Q%3D%3D&Button1=Button&TextBox1="
        Set oResponse = oConnection.Send(oRequest)
        If (oResponse is Nothing) Then
            Test.Trace "Error: Failed to receive response for URL to " + "/WebApplicationSampleCobol/WebForm1.aspx"
        Else
